...
class ReconstructionHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor, target_size: torch.Size) -> torch.Tensor:
        ...
        # Residual-size alignment for interpolation-based reconstruction.
        x = F.interpolate(x, size=target_size, mode="bilinear")
        x = self.proj(x)
        return x
...
class DualMCBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.d_model <= 0:
            raise ValueError("DualMCBlock requires at least one active branch")
        ...
        self.main_branch = BiMambaBlock(self.main_channels, expand=2) if self.main_channels > 0 else None
...
class DualMCN(nn.Module):
    ...
    A lightweight dual-branch architecture that combines:
    ...
    def __init__(
        ...
        use_stem_se: bool = False,
        use_block_se: bool = True,
        use_main_proj_bn: bool = True,
        use_aux_proj_bn: bool = True,
        use_fusion_proj_bn: bool = True,
        use_upsample_bn: bool = True,
