...
from dataset_loader import CSIDataset, robust_collate_fn
...
def build_model(model_name: str, device, input_size: int):
    cfg = get_model_config(model_name, input_size=input_size)
    ...
    return model, get_model_config(model_name, input_size=input_size)


def build_datasets(args):
    mapping_file = os.path.join(args.meta_root, args.mapping_file)
    train_split = os.path.join(args.meta_root, args.train_split)
    val_split = os.path.join(args.meta_root, args.val_split)
    test_id_split = os.path.join(args.meta_root, args.test_id_split)
    test_cross_env_split = os.path.join(args.meta_root, args.test_cross_env_split)
    ...
    common_kwargs = dict(
        ...
        target_subcarrier=args.target_subcarrier,
    )

    train_ds = CSIDataset(split_file=train_split, augment=True, **common_kwargs)
    ...
    if args.eval_mode in ["id", "both"]:
        test_id_ds = CSIDataset(split_file=test_id_split, augment=False, **common_kwargs)
...
def run_single_seed(
    ...
    model, model_config = build_model(model_name, device, input_size=input_size)
...
def main():
    parser = argparse.ArgumentParser("Baseline / final-candidate comparison runner for ourdata")
    parser.add_argument("--data_root", type=str, default="/mnt/F/tao/跌倒检测/new_data_processed_9to3")
    parser.add_argument("--meta_root", type=str, default="./meta_and_split")
    parser.add_argument("--mapping_file", type=str, default="label_mapping.json")
    parser.add_argument("--train_split", type=str, default="train_id.json")
    parser.add_argument("--val_split", type=str, default="val_id.json")
    parser.add_argument("--test_id_split", type=str, default="test_id.json")
    parser.add_argument("--test_cross_env_split", type=str, default="test_cross_env.json")
    parser.add_argument("--eval_mode", type=str, default="both", choices=["cross_env"])
    parser.add_argument("--save_dir", type=str, default="./outputs_model_compare_ourdata")
    parser.add_argument("--batch_size", type=int, default=16)
    parser.add_argument("--epochs", type=int, default=120)
    parser.add_argument("--lr", type=float, default=1e-3)
    parser.add_argument("--warmup_epochs", type=int, default=5)
    parser.add_argument("--weight_decay", type=float, default=1e-2)
    parser.add_argument("--target_len", type=int, default=1200)
    parser.add_argument("--target_subcarrier", type=int, default=114)
    ...
    input_size = 9 * args.target_subcarrier
    ...
    config = {
        ...
        "setting": "ourdata: e1+e2 -> train/val/test_id, e3 -> test_cross_env",
    }
