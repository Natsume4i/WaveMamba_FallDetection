...
def get_variant_config(variant_name: str):
    ...
    if variant_name == "dualstream_default":
        pass
    elif variant_name == "dualstream_no_wavelet":
        base["use_wavelet"] = False
    elif variant_name == "dualstream_main_only":
        base["branch_mode"] = "main_only"
    elif variant_name == "dualstream_aux_only":
        base["branch_mode"] = "aux_only"
    elif variant_name == "dualstream_recon_interp_1x1":
        base["recon_type"] = "interp_1x1"
    elif variant_name == "dualstream_no_stem_se_block_se":
        base["use_stem_se"] = False
        base["use_block_se"] = True
    elif variant_name == "dualstream_aux_pw":
        base["aux_variant"] = "mkconv_pw"
    else:
...
def build_model(variant_name: str, device):
...
def run_single_seed(
    variant_name, seed, args, device,
    train_loader, val_loader,
    test_id_loader, test_cross_env_loader,
    idx_to_label
):
    set_global_seed(seed)
    model, variant_config = build_model(variant_name, device)
    ...
    raw_ckpt_path = os.path.join(ckpt_dir, f"{variant_name}_seed{seed}_best_raw.pth")
    ema_ckpt_path = os.path.join(ckpt_dir, f"{variant_name}_seed{seed}_best_ema.pth") if args.use_ema else None
    ...
    test_id_metrics = None
    test_cross_env_metrics = None
...
def build_summary_rows(results_dict, eval_mode):
    rows = []
    for variant_name, payload in results_dict.items():
    ...
    if eval_mode == "id":
        rows = sorted(
            rows,
            key=lambda x: x.get("test_id_macro_f1_mean", -1.0) if x.get("test_id_macro_f1_mean") is not None else -1.0,
            reverse=True
        )
    else:
        rows = sorted(
            rows,
            key=lambda x: x.get("test_cross_env_macro_f1_mean", -1.0) if x.get("test_cross_env_macro_f1_mean") is not None else -1.0,
            reverse=True
        )
...
def save_md(summary_rows, out_path, use_ema=False, ema_decay=None):
    ...
    lines.append("")
    lines.append("| Variant | Params(M) | Best Val F1 | ID Acc | ID F1 | ID Fall R | ID Nonfall R | CrossEnv Acc | CrossEnv F1 | CrossEnv Fall R | CrossEnv Nonfall R | Best Epoch |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
...
# =========================================================
def main():
    parser = argparse.ArgumentParser("Unified ablation runner for WaveMamba (ID + Cross-Env + optional EMA)")
    ...
    parser.add_argument("--save_dir", type=str, default="./outputs_ablation_wavemamba_dual_eval_ema")
    ...
    parser.add_argument(
        ...
        default=[
            "dualstream_default",
            "dualstream_no_wavelet",
            "dualstream_main_only",
            "dualstream_aux_only",
            "dualstream_recon_interp_1x1",
            "dualstream_no_stem_se_block_se",
            "dualstream_aux_pw",
        ],
    )
    ...
    for variant_name in args.variants:
        ...
        for seed in args.seeds:
            print("\n" + "=" * 100)
            print(f"[Run] variant={variant_name} | seed={seed}")
            print("=" * 100)
            ...
            row, variant_config = run_single_seed(
                variant_name=variant_name,
                seed=seed,
                args=args,
                device=device,
            ...
            if "test_id_macro_f1" in row:
                msg += (
                    f" | id_f1={row['test_id_macro_f1']:.4f}"
                    f" | id_acc={row['test_id_accuracy']:.4f}"
                )
            if "test_cross_env_macro_f1" in row:
                msg += (
                    f" | cross_f1={row['test_cross_env_macro_f1']:.4f}"
                    f" | cross_acc={row['test_cross_env_accuracy']:.4f}"
                )
            print(msg)

        aggregate = aggregate_numeric_dicts(per_seed)
        final_results["variants"][variant_name] = {
            "variant_config": variant_config,
            "per_seed": per_seed,
            "aggregate": aggregate,
        }

        json_path = os.path.join(args.save_dir, "ablation_results.json")
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump(final_results, f, indent=2, ensure_ascii=False)

    summary_rows = build_summary_rows(final_results["variants"], args.eval_mode)

    if args.eval_mode == "id":
        final_results["ranking_by_test_id_macro_f1"] = [row["variant"] for row in summary_rows]
    else:
        final_results["ranking_by_test_cross_env_macro_f1"] = [row["variant"] for row in summary_rows]

    json_path = os.path.join(args.save_dir, "ablation_results.json")
    csv_path = os.path.join(args.save_dir, "ablation_summary.csv")
    md_path = os.path.join(args.save_dir, "ablation_summary.md")
