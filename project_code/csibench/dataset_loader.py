...
#        new_T = int(T * s)
#        data_1d = data.permute(0, 2, 1) 
#        stretched = F.interpolate(data_1d, size=new_T, mode='linear', align_corners=False)
#        stretched = stretched.permute(0, 2, 1)
#        if new_T > T:
#            start = (new_T - T) // 2
...
#            return F.pad(stretched, (0, 0, pad_left, pad_right), "constant", 0)
#
#    def shadowing(self, data):
#        data = data.clone()
#        C, T, S = data.shape 
#        seg_len = T // 3
#        if seg_len == 0: seg_len = T 
#        segments = [(0, seg_len), (seg_len, 2 * seg_len), (2 * seg_len, T)]
#        for (start, end) in segments:
#            if start >= T: break
#            if random.random() < 0.3:
#                drop_rate = random.uniform(0.1, 0.3)
#                num_drop = int(S * drop_rate)
#                if num_drop > 0:
#                    drop_indices = random.sample(range(S), num_drop)
#                    data[:, start:end, drop_indices] = 0   
#        return data
#
#    def __call__(self, sample):
#        if random.random() < self.p:
#            if random.random() < 0.5: sample = self.shadowing(sample)
#            if random.random() < 0.5: sample = self.time_stretching(sample)
#            if random.random() < 0.5: sample = self.random_amplitude_scale(sample)
#            if random.random() < 0.3: sample = self.gaussian_noise(sample)
#        return sample
